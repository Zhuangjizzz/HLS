...
int Parser::parse_function(std::string &line)
{
    char *tok;
    std::string key;
    char sep[] = " (,)";    //�Կո��������źͶ���Ϊ�ָ�������Щ���Ų��ᱻ����tok
    int cnt = 0;
    var p;          //���������������Ƿ�Ϊ����
    int pos;
    int parser_error = 0;

    tok = strtok(&line[0], sep);    //strtok��������ָ����һ���ַ�����ָ��

    while ((tok = strtok(NULL, sep)) != NULL)
    {
        cnt++;
        key = std::string(tok);
        if (key == "int" && cnt < 2)    //��һ���Ƿ���ֵ����
        {
            _ret_type = RET_INT;
        }
        else if (key == "void")
        ...
        else if (cnt > 2)   //���涼��
        {
            if (cnt % 2 == 0)   //varname || varname[]
            {
                ...
                {
                    p._name = key.substr(0, pos);   //��ȡkey[0:pos-1]
                    p._array_flag = true;
                }
...
int Parser::parse_statement(std::string &line, basic_block &bb)
{
    ...
    tok = strtok(&line[0], sep);    //line��һ���ַ���
    // 1. label
    key = std::string(tok);
    if (key.find(':') != std::string::npos) //��ǩ��
    {
        push_back_basic_block(bb);  //����һ�������õ�bb����
        bb.clear_statements();      //���
        std::string name = key.substr(0, key.length() - 1);
        bb.set_name(name);  //��ȡ��һ��block������
        label = 1;
    }
    else if (key == "store") // store
    {
        s.set_type(OP_STORE);
        s.set_num_oprands(3);   //store������������
        for (int i = 0; i < 3; ++i)
        {
            tok = strtok(NULL, sep);
            s.add_oprand(tok);  //������
    ...
    else if (key == "br") // br
    {
        int num_ops = 0;
        s.set_type(OP_BR);
        while ((tok = strtok(NULL, sep)) != NULL)   //br������д������������һ��
        {
            num_ops++;
            s.add_oprand(tok);
        }
    ...
    else    //������Щָ������ƶ����Ǵ����׿�ʼ��
    {
        ...
        keys.push_back(key);    //key�ǵȺ�������
        while ((tok = strtok(NULL, sep)) != NULL)
        {
            keys.push_back(tok);//����Ⱥţ��Լ��Ⱥ��Ҳ�ָ��Ͳ�����
        }
        if (keys[2] == "phi")
        {
            s.set_var(key);
            s.set_type(OP_PHI);
            s.set_num_oprands(keys.size() - 3); //����ǰ���������涼��
